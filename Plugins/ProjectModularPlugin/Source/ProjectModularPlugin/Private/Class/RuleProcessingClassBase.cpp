...
{
	FModulesMapWrapper ModulesMapWrapper;
	ModulesMapWrapper.NewModules = NewModules;
	
	// Sort
	TArray<FRule> DefaultRules = InitializeRuleProcessor(ModuleComponent, PreviousComponent, ModulesMapWrapper);
	
	TArray<FRule> AddedRules = AddRuleProcessor(ModuleComponent, PreviousComponent, ModulesMapWrapper);
	
	if (DefaultRules.Num() > 0 || AddedRules.Num() > 0)
	{
		...
		{
			DefaultRule.InvokeProcessingObject->InvokeProcessing(ModuleComponent, PreviousComponent, ModulesMapWrapper, DefaultRule);
		}
...
TArray<FRule> ARuleProcessingClassBase::InitializeRuleProcessor_Implementation(UModuleComponent* ModuleComponent, UModuleComponent* PreviousComponent, const FModulesMapWrapper& NewModules)
{
...
TArray<FRule> ARuleProcessingClassBase::AddRuleProcessor_Implementation(UModuleComponent* ModuleComponent, UModuleComponent* PreviousComponent, const FModulesMapWrapper& NewModules)
{
...
TArray<FRule> ARuleProcessingClassBase::AbbRuleProcessor_Implementation(const FModulesMapWrapper& NewModules)
{
	TArray<FRule> NewRules;
	return NewRules;
}
